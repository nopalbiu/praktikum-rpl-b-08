...
<body class="bg-background text-foreground min-h-screen flex flex-col">
    ...
    <main class="flex-grow container mx-auto p-4 md:p-8 mt-4">
        ...
        @if($cart && $cart->items->count() > 0)
            ...
            <div class="mt-6">
                @if($hasOutOfStock)
                    <button disabled class="block w-full bg-gray-600 text-gray-400 font-bold py-4 px-4 rounded text-lg text-center cursor-not-allowed opacity-60">
                        Buy — Beberapa item stok tidak mencukupi
                    </button>
                @else
                    <a href="{{ route('checkout.index') }}" class="block w-full bg-white hover:bg-gray-200 text-black font-bold py-4 px-4 rounded transition duration-200 text-lg text-center">
                        Buy
                    </a>
                @endif
            </div>
